...
func NewDeviceWithTimeout(Host string, Username string, Password string, DeviceType string, Port uint8, Timeout uint8, Options ...DeviceOption) (types.Device, error) {
	var device types.Device
	...
	//create connection
	connection, err := connections.NewConnection(Host, Username, Password, "ssh", Port, Timeout)
	if err != nil {
		return nil, err
	}
